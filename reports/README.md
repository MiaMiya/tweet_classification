...
We used a `requirements.txt` file for managing our dependencies. The list of dependencies was not autogenerated but manually written. It contains some external requirements, and local packages called with the `-e .`command. 
...
To retrieve the data locally, one would have to pull from google cloud storage with dvc, using the command:

`dvc pull`      

Finally to create the right python environment needed for running the code, one can run the create_environment function with `make` and afterward test it with test_environment. These two functions can be run with the following commands:

`make create_environment`

`make test_environment`

When running other functions such as training etc. the requirements are automatically run.
