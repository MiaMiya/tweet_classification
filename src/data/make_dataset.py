...
class Tweets(Dataset):
    def __init__(self, in_folder: str = "", out_folder: str = "") -> None:
        ...
        pd_combine = pd.concat(
            [pd_Russian[["Tweet", "Label"]], pd_D_T[["Tweet", "Label"]]],
            ignore_index=True,
    ...
    def load_preprocessed(self):
        try:
            ...
            data_train = np.load(
                "/gcs/tweet_classification/processed/train_processed.npy",
                allow_pickle=True,
            )
            data_test = np.load(
                "/gcs/tweet_classification/processed/test_processed.npy",
                allow_pickle=True,
            )
    ...
    def save_preprocessed(self):
        np.save(
            self.out_folder + "/train_processed.npy",
            np.stack((self.train_tweet, self.train_label)),
            allow_pickle=True,
        )
        np.save(
            self.out_folder + "/test_processed.npy",
            np.stack((self.test_tweet, self.test_label)),
            allow_pickle=True,
        )
