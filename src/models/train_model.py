        ...
        with tqdm(train_dl, unit="batch") as tepoch:
            ...
            for batch in train_dl:
                ...
                if device == torch.cuda.is_available():
                    acc_batch = accuracy(
                        log_probs.softmax(dim=-1).detach().cpu().flatten().numpy()
                        < 0.5,
                        batch["label"],
                    )
                else:
                    acc_batch = accuracy(
                        log_probs.softmax(dim=-1).detach().flatten().numpy() < 0.5,
                        batch["label"],
                    )
...
def train_main(lr, epoch, batch_size):
    ...
    # Load data
    data_set = Tweets(
        in_folder="/gcs/tweet_classification/raw",
        out_folder="/gcs/tweet_classification/processed",
    )
    # data_set = Tweets(in_folder="data/raw", out_folder="data/processed")
    data_set = Dataset.from_pandas(
        pd.DataFrame({"text": data_set.train_tweet, "label": data_set.train_label})
    )
    ...
    trainloader = DataLoader(
        tokenized_dataset, shuffle=True, batch_size=batch_size, collate_fn=collate_fn
    )
    ...
    scheduler = get_linear_schedule_with_warmup(
        optimizer, warmup_steps, epoch * len(trainloader)
    )

    # Train the model
    losses, acc = train(model, trainloader, optimizer, scheduler, epoch, device)
